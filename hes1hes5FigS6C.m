...
for i=1:numel(values)
      ...
      for j=1:numel(thresh)
          ...
          auc29(i,j)=sum(sol.y(1,:));
      end
end
%
figure,imagesc(flipud(auc29));
set(gca,'YTickLabel',[5:-0.25:0]);
set(gca,'XTickLabel',thresh);
ylabel('HES5 production rates');
...
for i=1:numel(values)
      ...
      for j=1:numel(thresh)
          par.p0_51=thresh(j);
          sol = dde23(@ddePM_coupled,tau,[0 1 0 1],[0, Tend],opt,par);
          auc60(i,j)=sum(sol.y(1,:));
      end
end
%
figure,imagesc(flipud(auc60));
set(gca,'YTickLabel',[5:-0.25:0]);
set(gca,'XTickLabel',thresh);
ylabel('HES5 production rates');
xlabel('Cross-repression threshold')
...
auc40=auc40/m;
auc60=auc60/m;
figure,subplot(1,3,1),imagesc(flipud(auc29))
colorbar, clim([0,1])
...
ylabel('Production rate');
xlabel('Cross-repression threshold')
title('Time delay 40min')
subplot(1,3,3),imagesc(flipud(auc60))
colorbar, clim([0,1])
set(gca,'YTick',[1:numel(values)]);
set(gca,'YTickLabel',[5:-0.25:0]);
set(gca,'XTick',[1:10]);
